...
Dias=input('¿Dias a planificar? '); %Dias a planificar.
...
m=3;%Dias a planificar en cada iteración.
...
while w < Dias 
     [u,r]=size (L);
    if r ~= 0
    [Op,Ocu,xmin,fval] = Planificacion(T_progr,L,m,Beta,Tiem_tot,Confi); %Planifica operaciones para m dias.
    matOpi=[];
    Fval=[Fval fval];
    for i=1:Sel
        Opi=[];
